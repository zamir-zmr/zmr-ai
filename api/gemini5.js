...
export default async function handler(req, res) {
  ...
  const apiKey = process.env.GEMINI_API_KEY_5 || process.env.GEMINI_API_KEY;
  if (!apiKey) {
    res.status(500).json({ error: { message: 'Server misconfigured: GEMINI_API_KEY_5 (ya GEMINI_API_KEY) missing' } });
    return;
  }
  ...
  // Frontend isko header se padhkar chat me sahi model-naam (Gemini 5)
